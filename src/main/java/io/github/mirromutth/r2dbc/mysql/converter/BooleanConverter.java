...
/**
 * Converter for {@link boolean}.
 */
final class BooleanConverter extends AbstractPrimitiveConverter<Boolean> {
    ...
    @Override
    public Boolean read(ByteBuf buf, boolean isUnsigned, int precision, int collationId, Class<? super Boolean> target, MySqlSession session) {
    ...
    boolean doCanRead(ColumnType type, boolean isUnsigned, int precision) {
        return ColumnType.BIT == type && precision == 1;
    }
}
